...
int execute_command_wrapper(const char *command, MDI_Comm mdi_comm, void* class_obj) {

	MDIEngine* engine_instance = (MDIEngine*)class_obj;
	engine_instance->execute_command(command, mdi_comm);
	return 0;
...
void MDIEngine::mdi_commands()
{
  ...
  MDI_Set_execute_command_func(execute_command_wrapper, this);
}
